$(document).ready(function () {
    ...
    if (window.location.pathname !== '/outdoor-monitor') return;
    ...
    function updateCards(d) {
        if (!d) return;
        $('#outdoor-dose-rate').html(d.usvh       !== null ? parseFloat(d.usvh).toFixed(2)       : '-');
        $('#outdoor-temperature').html(d.suhu      !== null ? parseFloat(d.suhu).toFixed(1)       : '-');
        $('#outdoor-humidity').html(d.kelembapan   !== null ? parseFloat(d.kelembapan).toFixed(1) : '-');
    }
    ...
    function startPolling() {
        ...
        $('#outdoor-live-indicator').removeClass('d-none');
    }

    function stopPolling() {
        if (pollInterval) { clearInterval(pollInterval); pollInterval = null; }
        $('#outdoor-live-indicator').addClass('d-none');
    }
            ...
            success: function (latest) {
                ...
                if (newTs > lastTs) {
                    ...
                    if (allData.length < prevLen) {
                        setOutdoorSeries(allData);
                    } else {
                        appendOutdoorPoint(latest);
                    }
                }
            },
    ...
    function loadData(rangeKey, customDate) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (allData.length === 0) {
                    setOutdoorNoData('Data Kosong');
                    return;
                }
                updateCards(allData[allData.length - 1]);
                setOutdoorSeries(allData);
                setOutdoorXRange(rangeKey, allData);
                setOutdoorAnnotations();
            },
    ...
    $('#outdoor-time-range').on('change', function () {
        var val = $(this).val();
        if (val === 'custom') {
            $('#outdoor-custom-date').removeClass('d-none');
            return;
        }
        $('#outdoor-custom-date').addClass('d-none');
        applyFilter(val);
    });

    $('#outdoor-custom-date').on('change', function () {
